...
contract Tide is ERC721Enumerable  {
  using Counters for Counters.Counter;
  Counters.Counter private _nextTokenId;

  uint256 constant PURCHASES_FOR_BOUNTY_PROMO = 3; // promo bounty mint every 3 purchases
  address public immutable bountyAddress; // deployed bounty contract address
  ...
  */
  ...
  function _checkMintOherBrandPromo(address wallet_) internal {
    if (_totalPurchases[wallet_] % PURCHASES_FOR_BOUNTY_PROMO == 0) {
      IBrand(bountyAddress).mintPromo(wallet_);
    }
  }
